export default {
  "background": "backgrounds/dispose_waste.png",
  "layers": [
    [],
    [
      {
        "type": "sprite",
        "source": "squirrel.png",
        "top": "43%",
        "left": "76%",
        "size": "8%",
        "dancing": true
      },
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "53%",
        "left": "46%",
        "size": "35%",
        "flip": false
      },
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "61%",
        "left": "39%",
        "size": "35%",
        "flip": false
      }
    ],
    [
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "53%",
        "left": "46%",
        "size": "35%",
        "flip": false
      },
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "61%",
        "left": "39%",
        "size": "35%",
        "flip": false
      }
    ],
    [
      {
        "type": "chat",
        "text": "Let's stop for a moment\nand have a snack!",
        "top": "44%",
        "left": "53%",
        "flip": false
      },
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "53%",
        "left": "46%",
        "size": "35%",
        "flip": false
      },
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "61%",
        "left": "39%",
        "size": "35%",
        "flip": false
      }
    ],
    [
      {
        "type": "chat",
        "text": "Good thing we\npacked plenty\nof food!",
        "top": "45%",
        "left": "25%",
        "flip": true
      },
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "61%",
        "left": "39%",
        "size": "35%",
        "flip": false
      },
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "53%",
        "left": "46%",
        "size": "35%",
        "flip": false
      }
    ],
    [
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "34%",
        "left": "60%",
        "size": "32%",
        "flip": true
      },
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "44%",
        "left": "56%",
        "size": "32%",
        "flip": true
      }
    ],
    [
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "34%",
        "left": "60%",
        "size": "32%",
        "flip": true
      },
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "44%",
        "left": "56%",
        "size": "32%",
        "flip": true
      }
    ],
    [
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "34%",
        "left": "60%",
        "size": "32%",
        "flip": true
      },
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "44%",
        "left": "50%",
        "size": "32%",
        "flip": false
      },
      {
        "type": "chat",
        "text": "Hey! You can't\nfeed any animals!",
        "top": "34%",
        "left": "30%",
        "flip": true
      }
    ],
    [
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "34%",
        "left": "60%",
        "size": "32%",
        "flip": true
      },
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "44%",
        "left": "50%",
        "size": "32%",
        "flip": false
      },
      {
        "type": "chat",
        "text": "Oh, really? Oops...",
        "top": "27%",
        "left": "67%",
        "flip": false
      }
    ],
    [
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "34%",
        "left": "60%",
        "size": "32%",
        "flip": true
      },
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "44%",
        "left": "50%",
        "size": "32%",
        "flip": false
      },
      {
        "type": "chat",
        "text": "Yup, it's harmful to the\nanimals to give them food\nthey don't naturally eat.",
        "top": "28%",
        "left": "22%",
        "flip": true
      }
    ],
    [
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "34%",
        "left": "60%",
        "size": "32%",
        "flip": true
      },
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "44%",
        "left": "50%",
        "size": "32%",
        "flip": false
      },
      {
        "type": "chat",
        "text": "Why so protective?",
        "top": "37%",
        "left": "30%",
        "flip": true
      }
    ],
    [
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "34%",
        "left": "60%",
        "size": "32%",
        "flip": true
      },
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "44%",
        "left": "50%",
        "size": "32%",
        "flip": false
      },
      {
        "type": "chat",
        "text": "It's harmful to the\nanimals to give them\nfood they don't\nnaturally eat.",
        "top": "17%",
        "left": "70%",
        "flip": false
      }
    ],
    [
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "34%",
        "left": "60%",
        "size": "32%",
        "flip": true
      },
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "44%",
        "left": "50%",
        "size": "32%",
        "flip": false
      }
    ],
    [
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "54%",
        "left": "46%",
        "size": "35%",
        "flip": true
      },
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "44%",
        "left": "56%",
        "size": "31%",
        "flip": true
      },
      {
        "type": "chat",
        "text": "Oh no! I can't leave\nmy wrapper on the trail!",
        "top": "43%",
        "left": "20%",
        "flip": true
      }
    ],
    [
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "34%",
        "left": "60%",
        "size": "32%",
        "flip": true
      },
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "54%",
        "left": "46%",
        "size": "35%",
        "flip": true
      },
      {
        "type": "chat",
        "text": "You can't leave\ntrash on the trail!",
        "top": "42%",
        "left": "23%",
        "flip": true
      }
    ],
    [
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "61%",
        "left": "38%",
        "size": "35%",
        "flip": false
      },
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "47%",
        "left": "50%",
        "size": "35%",
        "flip": true
      }
    ],
    [
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "47%",
        "left": "50%",
        "size": "35%",
        "flip": true
      },
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "61%",
        "left": "38%",
        "size": "35%",
        "flip": false
      }
    ],
    [
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "47%",
        "left": "50%",
        "size": "35%",
        "flip": true
      },
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "61%",
        "left": "38%",
        "size": "35%",
        "flip": false
      },
      {
        "type": "chat",
        "text": "I can't leave trash\nand you can't leave your \napple core either!",
        "top": "45%",
        "left": "10%",
        "flip": true
      }
    ],
    [
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "47%",
        "left": "50%",
        "size": "35%",
        "flip": true
      },
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "61%",
        "left": "38%",
        "size": "35%",
        "flip": false
      },
      {
        "type": "chat",
        "text": "Won't it decompose?",
        "top": "39%",
        "left": "56%",
        "flip": false
      }
    ],
    [
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "47%",
        "left": "50%",
        "size": "35%",
        "flip": true
      },
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "61%",
        "left": "38%",
        "size": "35%",
        "flip": false
      },
      {
        "type": "chat",
        "text": "That will take a long time\nand you're introducing plant\nmatter that doesn't belong here.\nThat's harmful to the plants and\nanimals!",
        "top": "40%",
        "left": "2%",
        "flip": true
      }
    ],
    [
      {
        "type": "sprite",
        "source": "characters/main-character.png",
        "top": "61%",
        "left": "38%",
        "size": "35%",
        "flip": false
      },
      {
        "type": "sprite",
        "source": "characters/green-jacket-kid.png",
        "top": "47%",
        "left": "50%",
        "size": "35%",
        "flip": true
      },
      {
        "type": "chat",
        "text": "I guess it's not that \ndifferent from the invasive\nspecies we just learned about!",
        "top": "31%",
        "left": "55%",
        "flip": false
      }
    ]
  ],
  "layerSteps": [
    {
      "index": 0
    },
    {
      "index": 1,
      "stepType": "Click through",
      "prompt": "You respond with:",
      "options": []
    },
    {
      "index": 2,
      "stepType": "Decision",
      "prompt": "You've been walking for a while, and your stomach starts to grumble.",
      "options": [
        {
          "layer": "3",
          "description": "Next"
        }
      ]
    },
    {
      "index": 3,
      "stepType": "Click through",
      "prompt": "You respond with:",
      "options": []
    },
    {
      "index": 4,
      "stepType": "Click through",
      "prompt": "You respond with:",
      "options": []
    },
    {
      "index": 5,
      "stepType": "Decision",
      "prompt": "You stop for a moment to enjoy the beautiful view.",
      "options": [
        {
          "layer": "6",
          "description": "Next"
        }
      ]
    },
    {
      "index": 6,
      "stepType": "Decision",
      "prompt": "A bird lands next to you and starts to go after your food.",
      "options": [
        {
          "layer": "7",
          "description": "Offer it bite of your snack"
        },
        {
          "layer": "10",
          "description": "Carefully keep your snack away from the bird"
        }
      ]
    },
    {
      "index": 7,
      "stepType": "Click through",
      "prompt": "You respond with:",
      "options": []
    },
    {
      "index": 8,
      "stepType": "Click through",
      "prompt": "You respond with:",
      "options": []
    },
    {
      "index": 9,
      "stepType": "Decision",
      "prompt": "",
      "options": [
        {
          "layer": "12",
          "description": "Next"
        }
      ]
    },
    {
      "index": 10,
      "stepType": "Click through",
      "prompt": "You respond with:",
      "options": []
    },
    {
      "index": 11,
      "stepType": "Click through",
      "prompt": "You respond with:",
      "options": []
    },
    {
      "index": 12,
      "stepType": "Decision",
      "prompt": "You accidentally drop your wrapper and it starts to blow away in the wind.",
      "options": [
        {
          "layer": "13",
          "description": "Run after it!"
        },
        {
          "layer": "14",
          "description": "Watch it blow farther down the trail..."
        }
      ]
    },
    {
      "index": 13,
      "stepType": "Decision",
      "prompt": "",
      "options": [
        {
          "layer": "15",
          "description": "Next"
        }
      ]
    },
    {
      "index": 14,
      "stepType": "Decision",
      "prompt": "Your friend runs after it",
      "options": [
        {
          "layer": "15",
          "description": "Next"
        }
      ]
    },
    {
      "index": 15,
      "stepType": "Decision",
      "prompt": "You put the wrapper in your pocket so you can find a proper trash can back at the park entrance.",
      "options": [
        {
          "layer": "16",
          "description": "Next"
        }
      ]
    },
    {
      "index": 16,
      "stepType": "Decision",
      "prompt": "Your friend finishes an apple and tosses the core off the side of the trail.",
      "options": [
        {
          "layer": "17",
          "description": "Next"
        }
      ]
    },
    {
      "index": 17,
      "stepType": "Click through",
      "prompt": "You respond with:",
      "options": []
    },
    {
      "index": 18,
      "stepType": "Click through",
      "prompt": "You respond with:",
      "options": []
    },
    {
      "index": 19,
      "stepType": "Click through",
      "prompt": "You respond with:",
      "options": []
    },
    {
      "index": 20,
      "stepType": "Click through",
      "prompt": "You respond with:",
      "options": []
    }
  ]
}
